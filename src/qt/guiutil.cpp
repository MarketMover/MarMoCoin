...
{
    // MarMoCoin: check prefix
    if(uri.scheme() != QString("MarMoCoin"))
        return false;
...
{
    // Convert MarMoCoin:// to MarMoCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("MarMoCoin://"))
    {
        uri.replace(0, 12, "MarMoCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MarMoCoin.lnk";
}
...
{
    return GetAutostartDir() / "MarMoCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MarMoCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MarMoCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MarMoCoin-Qt"));
